...
class BindDialog : BaseSheetDialog() {
    ...
    companion object {
        private const val SMS_URL =
            "https://www.zhixue.com/container/app/common/user/sendSMS4Mobile" //验证码URL

        private const val BIND_URL =
            "https://www.zhixue.com/container/app/common/user/modifyMobile" //绑定手机URL
    }
    ...
    ): View = inflater.inflate(R.layout.dialog_bind, container)
        ...
        bindCodeBtn.setOnClickListener {
            if (bindMobileNumber.isNotEmpty()) launch {
                activity!!.api(SMS_URL + smsParams, null, true, JsonObject).let {
                    withContext(Main) {
                        if (it is JSONObject) context!!.toast(R.string.smsSuccess)
